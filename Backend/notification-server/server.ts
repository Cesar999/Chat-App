...
async function addContactDB(user:any, contact:any){
    ...
    let contact_id = await User.findOne(contact)
        .then((c)=>{
            if(c){
                console.log(c._id);
                return c._id;
    ...
    let contacts_arr = await User.findOne(user)
        .then((user)=>{
            if(user){
    ...
    .then(async (user: any)=>{
        if(user){
            ...
            for(let c of user.contacts){
                let conv_id = await getConversationId(user._id, c._id);
...
async function getConversationId(user_id: any, contact_id:any){   
    let conv_id = await Conversation.findOne({participants:{$in:[user_id, contact_id]}})
    .then((c)=>{
        console.log(c);
        return c._id;
    });
